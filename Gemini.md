...
*   **启动命令**:
    ...
    sudo mkdir -p /var/lib/giantvm/chunks
    sudo chmod 777 /var/lib/giantvm/chunks
    # 2. 定义本节点共享内存标识（用于防止单机部署时内存重叠坍缩，多机部署可不使用该项设置，使用默认的 /giantvm_ram）
    export GVM_SHM_FILE="/giantvm_ram_node0"
    # 3. 加载内核模块
    sudo insmod giantvm.ko 
    service_port=9000 
    local_slave_port=9005
    # 4. 启动 Master
    # <RAM_MB> <L_PORT> <CONF> <ID> <C_PORT> <SLAVE_PORT> <SYNC_BATCH>
    ./giantvm_node_master 4096 9002 /etc/giantvm/logical_topology.txt 0 9001 9005 64 &
    # 5. 等待 SHM 创建与预热
    sleep 2
    # 6. 启动 Slave
...
qemu-system-x86_64 \
  -name "GVM-Wavelet-SSI-Core" -m 136G -smp 72 \
  ...
  -device giantvm-gpu-stub,id=gpu_remote,bus=rp1,vendor_id=0x10de,device_id=0x1eb8,bar1_size=16G \
  \
  -nographic -vga none
```
...
*   **配置文件 (`/etc/giantvm/fractal_vm.cfg`)**:
    ```ini
    [machine]
...
*   **启动命令**:
    ...
    export GVM_ENV_SOCK_PATH=$(strings /proc/$(pgrep -f giantvm_node_master)/environ | grep GVM_ENV_SOCK_PATH | cut -d= -f2)
    qemu-system-x86_64 \
      -name "GVM-Fractal-SuperComputer" \
      -m 16T -smp 4096 \
      -readconfig /etc/giantvm/fractal_vm.cfg \
      -nographic -vga none
    ```
...
void gvm_prophet_abi_scanner(gvm_kvm_context_t *ctx) {
    ...
    if (ctx->rax == 0 && acc_len >= 1048576) {
        // 使用文件中真实存在的广播接口
        struct gvm_rpc_batch_memset batch = { .val = htonl(0), .count = htonl(1) };
        struct gvm_rpc_region region = { .gpa = GVM_HTONLL(aligned_start), .len = GVM_HTONLL(acc_len) };
        uint8_t buf[sizeof(batch) + sizeof(region)];
        memcpy(buf, &batch, sizeof(batch));
        memcpy(buf + sizeof(batch), &region, sizeof(region));
        
        gvm_logic_broadcast_rpc(buf, sizeof(buf), MSG_RPC_BATCH_MEMSET);
...
int gvm_rpc_call(uint16_t msg_type, void *payload, int len, uint32_t target_id, void *rx_buffer, int rx_len) {
    ...
    if (msg_type == MSG_VCPU_RUN && rx_buffer != NULL) {
        struct gvm_ipc_cpu_run_ack *run_ack = (struct gvm_ipc_cpu_run_ack *)rx_buffer;
        
        // 激活！调用你前面定义的 scanner 
        if (run_ack->status == 0 && run_ack->mode_tcg == 0) {
            // 这才是 logic_core 应该有的样子：职责分离
            gvm_prophet_abi_scanner(&run_ack->ctx.kvm);
        }
    }
...
void gvm_logic_process_packet(struct gvm_header *hdr, void *payload, uint32_t source_node_id) {
    ...
    switch(type) {
        ...
        case MSG_PING: {
            ...
            size_t ack_len = sizeof(struct gvm_header);
 ...
 * [关键逻辑] 在 9001 独立控制端口（可自行配置)上监听 GVMC 指令，通过写锁（Wrlock）强制注入跨 Pod 的联邦路由。
