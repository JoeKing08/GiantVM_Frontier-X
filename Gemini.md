...
*   **发起者**: Node 0 **Master 进程** (`giantvm_node`)。
*   **接收者**: Node 2 **Master 进程** (`giantvm_node`)。
*   **逻辑路径**:
...
*   **处理者**: Node 1 **Slave 进程** (`giantvm_slave`)。
*   **逻辑路径**:
...
*   **发起者**: Node 0 **Master 进程** (`giantvm_node`)。
*   **处理者**: Node 999 **Master 进程** (`giantvm_node`)。
*   **全路径追踪**:
...
*   **发起者**: Node 0 **Master 进程**。
*   **处理者**: Node 500 **Slave 进程** (`giantvm_slave`)。
*   **全路径追踪**:
...
**新物理法则**：由于 `giantvm_node` (Master) 采用了 **“2MB 兴趣订阅粒度 (GVM_AFFINITY_SHIFT=21)”**，我们要求 QEMU 启动参数必须严格遵循 **2MB 整数倍对齐**。
